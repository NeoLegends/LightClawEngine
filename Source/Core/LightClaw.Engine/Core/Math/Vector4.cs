...
namespace LightClaw.Engine.Core
{
    ...
    public struct Vector4 : ICloneable, IEquatable<Vector4>, IComparable<Vector4>
    {
        ...
        public Vector4(System.Drawing.Color color) : this(Color.ToFloat(color.R), Color.ToFloat(color.G), Color.ToFloat(color.B), Color.ToFloat(color.A)) { }

#endif

        /// <summary>
        /// Creates a new <see cref="Vector4"/> from the given <see cref="Vector3"/> and the <see cref="Double"/>.
        /// </summary>
        ...
        public Vector4(Vector4d vector) : this((float)vector.X, (float)vector.Y, (float)vector.Z, (float)vector.W) { }
        ...
        public Vector4(Color color) : this(Color.ToFloat(color.R), Color.ToFloat(color.G), Color.ToFloat(color.B), Color.ToFloat(color.A)) { }
        ...
        {
            return Vector4.Normalize(ref this);
        }
        ...
        public static Vector4 CatmullRom(ref Vector4 value1, ref Vector4 value2, ref Vector4 value3, ref Vector4 value4, float amount)
        {
            ...
            return new Vector4()
            {
                ...
                W = 0.5f * ((((2.0f * value2.W) + ((-value1.W + value3.W) * amount))
        ...
        public static void Orthogonalize(Vector4[] destination, params Vector4[] source)
        {
            ...
            for (int i = 0; i < source.Length; ++i)
            {
                ...
                for (int r = 0; r < i; ++r)
                {
                    newvector -= destination[r] * (Vector4.Dot(ref destination[r], ref newvector) / Vector4.Dot(ref destination[r], ref destination[r]));
                }
        ...
        public static void Orthonormalize(Vector4[] destination, params Vector4[] source)
        {
            ...
            for (int i = 0; i < source.Length; ++i)
            {
                ...
                for (int r = 0; r < i; ++r)
                {
                    newvector -= destination[r] * Vector4.Dot(ref destination[r], ref newvector);
                }
        ...
        /// </remarks>
        public static Vector4 Reflect(ref Vector4 vec, ref Vector4 surfaceNormal)
        {
            float dot = Vector4.Dot(ref vec, ref surfaceNormal);

            return new Vector4()
            {
                X = vec.X - ((2.0f * dot) * surfaceNormal.X),
                Y = vec.Y - ((2.0f * dot) * surfaceNormal.Y),
                Z = vec.Z - ((2.0f * dot) * surfaceNormal.Z),
                W = vec.W - ((2.0f * dot) * surfaceNormal.W)
            };
        }
        ...
        public static explicit operator Vector4(OpenTK.Vector4d vector)
        {
            return new Vector4((float)vector.X, (float)vector.Y, (float)vector.Z, (float)vector.W);
        }

        public static explicit operator OpenTK.Vector4d(Vector4 vector)
        {
